...
# Setup for transit time tracking
rbc_start_times = {}         
transit_times = []           # list of completed transit times
completed_rbc_ids = set()    # RBCs recorded
...
def update(frame):
    ...
    for idx, ((start_node, end_node), pos) in enumerate(RBC_location):
        ...
        if do_bifurcate:
            ...
            if current_node == n * n - 1:  # Special case: top-right corner
                if idx not in completed_rbc_ids:
                    end_time = frame * 0.005  
                    if idx in rbc_start_times:
                        start_time = rbc_start_times[idx]
                        transit_times.append(end_time - start_time)
                        completed_rbc_ids.add(idx)
                        if len(transit_times) == 100: # Number of RBCs tracked before stopping
                            print(transit_times, frame * 0.005 / tau)

                current_node = 0  # Reset to the bottom-left corner
                rbc_start_times[idx] = frame * 0.005
    ...
    norm = plt.Normalize(vmin=np.min(p_vec), vmax=np.max(p_vec))
    colors = cm.jet(norm(p_vec))
    node_colors = [colors[i] for i in range(num_nodes)]
    node_scatter.set_color(['blue' for _ in range(num_nodes)])
...
# Animation
ani = FuncAnimation(fig, update, frames=100000, interval=0.05, blit=False)
plt.show()

plt.hist(transit_times, bins=20)
plt.xlabel("Transit Time (s)")
plt.ylabel("Number of RBCs")
plt.title("Distribution of RBC Transit Times")
plt.show()
