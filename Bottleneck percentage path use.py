...
G_norm = plt.Normalize(vmin=np.min(G_values_init), vmax=np.max(G_values_init)) 
sm = plt.cm.ScalarMappable(cmap=cm.jet, norm=G_norm)
sm.set_array([])
cbar = plt.colorbar(sm, ax=ax)
cbar.set_label('Local Pressure Gradient (G)')

N_total = np.zeros((num_nodes, num_nodes))

rbc_paths = [{'active': False, 'edges': []} for _ in range(num_RBC)]
valid_rbc_count = [0]  
edge_pass_counts = np.zeros((num_nodes, num_nodes))
...
def update(frame):
    ...
    for idx, ((start_node, end_node), pos) in enumerate(RBC_location):
        ...
        if do_bifurcate:
            ...
            # Activate RBC tracking when it passes through node 0
            if start_node == 0 and not rbc_paths[idx]['active']:
                rbc_paths[idx]['active'] = True
                rbc_paths[idx]['edges'] = [(start_node, end_node)]
            elif rbc_paths[idx]['active']:
                rbc_paths[idx]['edges'].append((start_node, end_node))



            if current_node == num_nodes - 1:  # RBC reached outflow node
                if rbc_paths[idx]['active']:
                    valid_rbc_count[0] += 1
                    for i, j in set(rbc_paths[idx]['edges']):
                        edge_pass_counts[i][j] += 1
                        edge_pass_counts[j][i] += 1
                    rbc_paths[idx]['active'] = False
                    rbc_paths[idx]['edges'] = []
                current_node = 0  # reset for re-entry
            ...
            N_total[next_edge[0], next_edge[1]] += 1
            N_total[next_edge[1], next_edge[0]] += 1
    ...
    if np.isclose(frame * 0.005 / tau, 10, atol=1e-3):
        if valid_rbc_count[0] > 0:
            edge_proportions = edge_pass_counts / valid_rbc_count[0]
        else:
            edge_proportions = np.zeros_like(edge_pass_counts)

        for i, j in edges:
            print(f"Edge ({i},{j}): {100 * edge_proportions[i][j]:.2f}%")
        print(valid_rbc_count[0])
